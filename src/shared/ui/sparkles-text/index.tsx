...
export const SparklesText: ParentComponent<SparklesTextProps> = (props) => {
  const [_localProps, forwardProps] = splitProps(props, ["children", "class", "colors", "sparklesCount"]);
  const localProps = mergeProps({ colors: ["#9E7AFF", "#FE8BBB"], sparklesCount: 10 }, _localProps);
  ...
  onMount(() => {
    const generateStar = (): Sparkle => {
      ...
      return { id, x: starX, y: starY, color: color!, delay, scale, lifespan };
    };
  ...
  return (
    <span class={styles({ class: localProps.class })} {...forwardProps}>
